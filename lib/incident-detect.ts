...
// - 「〜してしまいそう」（未遂）
// - 「〜してしまいましたか？」（文末の疑問。「〜しまいましたから」は完了なので除外しない）
// - 「〜してしまったら」（仮定。「〜してしまったらしく」は伝聞の完了なので除外しない）
// - 「〜してしまってはいけない／〜してしまってもいい」（規範・許可の述語が続く場合だけ。
//   「〜してしまってはじめて」のような完了叙述は除外しない）
const COMPLETED_RE =
  /(振り込ん|振込ん|送金し|渡し|払っ|支払っ|入金し|入力し|教え|伝え|送っ|提出し|登録し|インストールし|入れ)(て|で)しま(い(?!そう|ました(?:か(?=$|[?？。！、\s」』と])|\?|？|よね))|っ(?!た(?:ら(?!し)|なら)|て(?:は(?=いけ|いや|だめ|ダメ|なら|まず)|も(?=いい|よい|かまい|構い|大丈夫|問題))))/;
...
const EXPLICIT_DONE_RE =
  /(すでに|もう)[^。、]{0,6}(振り込みました|振り込んだ|送金しました|送金した|支払いました|支払った|払いました|払った|渡しました|渡した|教えました|教えた|送りました|送った|入力しました|入力した)(?!(?:か(?!ら)|よね|\?|？))/;
