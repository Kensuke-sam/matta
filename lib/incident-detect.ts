...
const EXPLICIT_DONE_RE =
  /(すでに|もう)[^。、]{0,6}(振り込みました|振り込んだ|送金しました|送金した|支払いました|支払った|払いました|払った|渡しました|渡した|教えました|教えた|送りました|送った|入力しました|入力した)/;
...
export function detectCompletedIncident(input: ValidatedInput): boolean {
  ...
  if (doneAnswer && AFFIRMATIVE_RE.test(doneAnswer.answer.normalize("NFKC").trim())) {
    return true;
  }
  return false;
}
